---
name: fiscus
description: Monthly credit card / bank statement review. Use when user exports a PDF statement or says "check statements", "financial review", "process statement". Parses PDF, checks recurring charges against baseline, flags anomalies, sends Telegram digest.
user_invocable: true
---
...
- **PDF drop folder:** iCloud Drive — confirm exact path on first run (`~/Library/Mobile Documents/com~apple~CloudDocs/`). Update this skill once confirmed.
...
User exports statement PDF from SC/HSBC app → saves to iCloud Drive folder → says "process statement" or "/fiscus".
...
```bash
# Find most recent statement PDF in iCloud Drive
ls -lt ~/Library/Mobile\ Documents/com~apple~CloudDocs/Statements/ | head -5
```
...
- Do NOT store raw statement PDFs anywhere except the iCloud folder Terry chose.
