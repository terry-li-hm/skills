...
| **Peer pattern transfer** | Scan what practitioners in your space are doing. Extract the technique, not the product. Route to every domain it applies to. Counter to NIH syndrome. Operationalised: `specula` skill. | Feeling behind, entering new domain, "what are others doing?", monthly review, competitor analysis |
| **Review cost trap** | Delegation lowers production cost but increases coordination/review cost — and that falls on the human. 47% more PRs but review fatigue rises proportionally. Not all output needs equal review: triage by risk. | Delegate-first workflows, feeling overwhelmed by output, "I can't review all this", scaling AI-assisted work |
| **Scaffolding load test** | Before building elaborate infrastructure, validate it's load-bearing. 100 lines + bash + capable model = 74% on SWE-bench. The complexity often lives in the model, not the scaffolding. Counter to over-engineering. | Adding skills, hooks, frameworks, "do we need this?", infrastructure decisions, tool proliferation |
| **Co-design over generic** | Tools optimized for a specific model outperform model-agnostic tools. Optimize descriptions, error formats, and schemas for how your model reasons, not for any model. | Building tools/MCP servers, writing prompts, designing agent interfaces |
