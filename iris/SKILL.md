---
name: iris
description: Email verification link relay — polls Gmail for a verification email, extracts the https link, and opens it in an agent-browser tab. Use for same-browser-session 2FA flows (Stripe key rotation, financial portals, etc.) where the site emails a link that must be opened in the same browser context.
---
