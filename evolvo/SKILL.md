---
name: evolvo
description: Session wrap quality analyzer — scan Claude Code session JSONL files to extract wrap outputs, compute multi-wrap rate, and sample output quality. Use during monthly skill-review (Step 5) or when wrap quality feels off.
---

# evolvo

Rust CLI that reads `~/.claude/projects/-Users-terry/*.jsonl`, extracts `─── Wrap` blocks, and produces stats + a spread sample for quality review.

## Install

Binary at `~/bin/evolvo`. Source: `~/code/evolvo/`.
...
- Binary is in workspace target (`~/code/target/release/evolvo`), not `~/code/evolvo/target/` — the parent `~/code/Cargo.toml` is a workspace.
