...
- Do NOT surface work priorities, TODO, calendar, or inbox — that's `/interphase` (evening) or `/ultradian` (ad-hoc)
- Do NOT create or edit vault notes
- Do NOT run Oura trend analysis — that's `/ecdysis` health section
- Keep it under 60 seconds of reading. Morning is for glancing, not studying.

## See also
- `/interphase` — the one daily routine (evening)
