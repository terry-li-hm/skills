...
10b. **Pre-meeting dossiers** — run `amicus dossier --today 2>/dev/null`. If output is non-empty, weave attendee context (name, last contact, recent subjects) naturally into the Today's plate narrative. Fail silently if amicus is not installed or DB is empty — do not block the brief.
